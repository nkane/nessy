...
;
; Depends on the per-scanline A12 clock: BG pattern table at $0000 +
; sprite pattern table at $1000 (PPUCTRL bit 3) means the per-line
; garbage sprite-pattern fetches read $1xxx and toggle A12, so MMC3
; counts scanlines even with no sprites on screen — exactly how real
; MMC3 games arm the scanline IRQ.
;
...
:       bit     $2002
        ...
        ; clear); sprite pattern table goes to $1000 (bit 3) below so the
        ; per-line garbage sprite fetches drive a clean A12 rise.
        lda     #$08
        sta     $2001
        ...
        ; Enable NMI on vblank ($2000 bit 7) + sprite pattern table $1000
        ; ($2000 bit 3). BG pattern table = $0000 (bit 4 clear).
        lda     #$88
        sta     $2000
        cli
:       jmp     :-
